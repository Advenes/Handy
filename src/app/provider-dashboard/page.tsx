...
const ProviderDashboardPage = () => {
  ...
  const getPriorityBorder = (priority: JobPriority) => {
    switch (priority) {
      ...
      <div className="min-h-screen bg-gray-50">
        <div className="mx-auto max-w-5xl">
          ...
          <div className="relative overflow-hidden bg-gradient-to-r from-[#FF7A00] to-[#FFB347] shadow-sm">
            <div className="px-6 py-6">
              <div className="flex items-center justify-between">
                <div>
                  <h1 className="text-2xl font-semibold text-white">Dostępne zadania</h1>
                ...
                <div className="flex gap-2">
                  <Link
                    href="/"
                    className="px-4 py-2 bg-white/10 border border-white/30 text-white rounded-lg text-sm font-medium hover:bg-white/20 transition-colors"
                  >
                    Main Page
                  </Link>
                  <button
                    onClick={handleLogout}
                    className="px-4 py-2 bg-white text-[#FF7A00] rounded-lg text-sm font-medium hover:bg-white/90 transition-colors"
                  >
                    Wyloguj się
                  </button>
                </div>
          ...
          <div className="px-6 mt-4">
            <div className="flex items-center justify-between rounded-xl bg-white border border-orange-100 p-4 shadow-sm">
              <div className="flex items-center gap-3">
                <span className="text-2xl">🔥</span>
                <p className="text-sm text-gray-700"><span className="font-semibold text-[#FF7A00]">5 nowych zleceń</span> w Twojej okolicy od ostatniej wizyty</p>
              </div>
              <button className="px-3 py-2 text-sm bg-[#FF7A00] text-white rounded-lg hover:bg-[#E86A00] transition">Odśwież</button>
            </div>
          </div>
          ...
          <div className="px-6 py-6">
            ...
            <div className="space-y-4">
              {filteredJobs.map((job) => (
                <div key={job.id} className={`bg-white rounded-xl p-6 shadow-sm border border-gray-100 hover:shadow-md transition-shadow border-l-4 ${getPriorityBorder(job.priority)}`}>
